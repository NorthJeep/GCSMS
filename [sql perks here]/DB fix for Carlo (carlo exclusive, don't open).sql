...
CREATE DATABASE IF NOT EXISTS pupqcdb;
USE pupqcdb;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `student_profiling`  AS  select `r_stud_profile`.`Stud_NO` AS `STUD_NO`,concat(`r_stud_profile`.`Stud_FNAME`,' ',`r_stud_profile`.`Stud_LNAME`) AS `FULLNAME`,concat(`r_stud_profile`.`Stud_COURSE`,' ',`r_stud_profile`.`Stud_YEAR_LEVEL`,' - ',`r_stud_profile`.`Stud_SECTION`) AS `COURSE`,`r_stud_profile`.`Stud_STATUS` AS `STUD_STATUS` from `r_stud_profile` where ((`r_stud_profile`.`Stud_STATUS` = 'Regular') or (`r_stud_profile`.`Stud_STATUS` = 'Irregular')) ;
