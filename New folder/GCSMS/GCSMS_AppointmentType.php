<?php
    ...
    if (isset($_GET['row']) && isset($_GET['action'])) 
    {
        ...
        $query = "UPDATE r_couns_appointment_type SET Appmnt_STAT = 'Inactive' WHERE Appmnt_TYPE = '$type'";

        $result = mysqli_query($connect,$query);

        mysqli_close($connect);
        header('location: GCSMS_AppointmentType.php');
    }
?>
                ...
                <section class="panel">
                    ...
                    <table  class="display table table-bordered table-striped" id="dynamic-table">
                        ...
                        <tbody id="appmnt">
                            <?php
                                ...
                                if ($records) 
                                {
                                    while ($row = $records -> fetch_assoc()) 
                                    {
                                        echo '
                                            <tr>
                                                <td id="appmnt-type">'.$row["Appmnt_TYPE"].'</td>
                                                <td id="appmnt-stat">'.$row['Appmnt_STAT'].'</td>
                                                <td class="center">
                                                    <button type="submit" class="btn btn-info" ><i class="fa fa-pencil-square-o" name="edit"></i></button>
                                                    <a id="delete" href="#deactivateModal" data-toggle="modal" class="btn btn-danger delete"><i class="fa fa-check"></i></a>
                                                </td>
                                            </tr>
                                        ';
                                    }
                                ...
                                //     $type = $appointment['Appmnt_TYPE'];
                                //     $stat = $appointment['Appmnt_STAT'];

                                    // echo "<tr>";
                                    //     echo "<td>".$appointment['Appmnt_TYPE']."</td>";
                                    //     echo "<td>".$appointment['Appmnt_STAT']."</td>";
                            ...
                            <!-- <tr>
                                <td id="visit-type"> <?php  $type; ?> </td>
                                <td> <?php  $stat; ?> </td>
                                <td class="center">
                                    <button type="submit" class="btn btn-info"><i class="fa fa-pencil-square-o" name="update"></i></button>
                                    <a id="delete" data-toggle="modal" class="btn btn-danger delete" href="#deactivateModal"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr> -->

                            <!--Delete Modal-->
                                <div id="deactivateModal" aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1"  class="modal fade">
...
<script type="text/javascript">
    $("#appmnt").on("click","a[id='delete']",function()
    {
        $type = $(this).parent().siblings("td[id='appmnt-type']").text();
        $stat = $(this).parent().siblings("td[id='appmnt-stat']").text();
        $("#modalform").attr("action","?row="+$type+"&action=deactivate");
    });
</script>
